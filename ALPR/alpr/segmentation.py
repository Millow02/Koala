...
class Segmentation:
    ...
    def set_directories(self, cropped_dir, rectified_dir, output_dir):
        ...
        if self.cropped_directories and self.output_dir and self.rectified_dir:
            log("SEGMENTATION", f"Input directories: {self.cropped_directories} & {self.rectified_dir} | Output directory: {self.output_dir} ")
    ...
    def upscale_input(self, image_path): 
        ...
        if image is None:
            ...
            return None
        upscaled_image = self.upscaler.upscale_image(image, ( self.input_width, self.input_height))
    ...
    def segment_characters(self, image_path, output_dir, zoom):
        ...
        for result in results:
            ...
            for idx, box in enumerate(boxes):
                ...
                crop = image[max(y1 - zoom, 0):min(y2 + zoom, image.shape[0]),
    ...
    def process_cropped_images(self):
        for directory in self.cropped_directories:
            ...
            for zoom in self.padding_levels:
                ...
                n_char_segmented = self.segment_characters(crop_filename, output_seg_dir, zoom)
                log("SEGMENTATION",
